...
export const actions = {
  ...
  // FIELDS
  // -----------------------------
  async getFields ({commit}, params = {page: 1}) {
    commit('fields/REQUEST_LIST')
      const {data} = await this.$axios.get(`/apps/${this.getters.appId}/fields`, {params})
      if (data && data.errno === 0) {
        const isFirstPage = params.page && params.page > 1
        const commitName = `fields/${isFirstPage ? 'ADD' : 'GET'}_LIST_SUCCESS`
        commit(commitName, data)
      } else {
        commit('fields/GET_LIST_FAILURE')
      }
  }
  // async loadUsers ({commit}, params = {page: 1}) {
  //   commit('users/REQUEST_LIST')
